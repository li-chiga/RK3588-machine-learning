                                ...
                                struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->id.vendor = ts->vendor_id;
    ts->input_dev->id.product = ts->product_id;
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (of_property_read_u32(np, "vendor-id", &ts->vendor_id))
		ts->vendor_id = 0xDEAD;
	if (of_property_read_u32(np, "product-id", &ts->product_id))
		ts->product_id = 0xBEEF;
